...
class GoogleSpreadsheetCell {
  ...
  _updateRawData(newData = {}) {
    ...
    this._error = null;
    if (get(this._rawData, 'effectiveValue.errorValue')) {
  ...
  get value() {
    ...
    return _values(this._rawData.effectiveValue)[0];
  }
  ...
  _getUpdateRequest() {
    ...
    const isFormatUpdated = !!keys(this._draftData.userEnteredFormat || {})
      .length;
    ...
    if (
      !some([isValueUpdated, isNoteUpdated, isFormatUpdated, isFormatCleared])
    ) {
      return null;
    }
