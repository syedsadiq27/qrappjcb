...
import { getFieldMask, columnToLetter, letterToColumn } from './utils';

function checkForDuplicateHeaders(headers) {
  // check for duplicate headers
  const checkForDupes = groupBy(headers); // { c1: ['c1'], c2: ['c2', 'c2' ]}
...
class GoogleSpreadsheetWorksheet {
  ...
  get cellStats() {
    ...
    return {
      nonEmpty: _filter(allCells, c => c.value).length,
  ...
  async loadCells(sheetFilters) {
    ...
    let filtersArray = isArray(sheetFilters) ? sheetFilters : [sheetFilters];
    filtersArray = map(filtersArray, filter => {
  ...
  async saveUpdatedCells() {
    const cellsToSave = _filter(flatten(this._cells), { _isDirty: true });
  ...
  async loadHeaderRow(headerRowIndex) {
    ...
    this.headerValues = map(rows[0], header => header.trim());
  ...
  async addRows(rows, options = {}) {
    ...
    return map(response.data.updates.updatedData.values, rowValues => {
  ...
  async insertDimension(columnsOrRows, range, inheritFromBefore = null) {
    ...
    if (!isInteger(range.endIndex) || range.endIndex < 0)
